...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda authorizer for WebSocket connect requests that validates both API keys and Cognito JWT tokens.

    Expected event structure:
    {
        "type": "TOKEN",
        "methodArn": "arn:aws:execute-api:...",
        "authorizationToken": "Bearer jwt-token" OR "api-key-here",
        "queryStringParameters": {
            "token": "jwt-token" OR
            "api_key": "your-api-key" OR
    ...
    try:
        # For WebSocket authorizers, the token can be in authorizationToken or query parameters
        auth_token = event.get("authorizationToken", "")
        query_params = event.get("queryStringParameters", {}) or {}

        # Check for JWT token authentication first in query parameters
        auth_param = query_params.get("authorization") or query_params.get("token")
        if auth_param and auth_param.startswith("Bearer "):
            token = auth_param.split(" ")[1]
            logger.info("Attempting JWT token authentication from query parameters")
            ...
            if user_info:
                ...
                logger.warning(
                    f"Invalid JWT token provided in query parameters: {auth_param}"
                )
        ...
        if api_key:
            logger.info("Attempting API key authentication from query parameters")
            if validate_api_key(api_key):
                logger.info("API key validation successful from query parameters")
                return generate_policy("api-key-user", "Allow", event["methodArn"])
            else:
                logger.warning(
                    f"Invalid API key provided in query parameters: {api_key[:8]}..."
                )
                raise Exception("Invalid API key")

        # Check for token in authorizationToken field (fallback)
        if auth_token:
            if auth_token.startswith("Bearer "):
                token = auth_token.split(" ")[1]
                logger.info(
                    "Attempting JWT token authentication from authorizationToken"
                )

                user_info = validate_jwt_token(token)
                if user_info:
                    logger.info(  # Decode token header to get key ID
                        f"JWT validation successful for user: {user_info.get('email', 'unknown')}"
                    )
                    return generate_policy(
                        user_info.get("sub", "jwt-user"),
                    ...
                    logger.warning("Invalid JWT token provided in authorizationToken")
                    raise Exception("Invalid JWT token")
            else:
                # Treat as API key
                logger.info("Attempting API key authentication from authorizationToken")
                if validate_api_key(auth_token):
                    logger.info("API key validation successful from authorizationToken")
                    return generate_policy("api-key-user", "Allow", event["methodArn"])
                else:
                    logger.warning(
                        f"Invalid API key provided in authorizationToken: {auth_token[:8]}..."
                    )
                    raise Exception("Invalid API key")
